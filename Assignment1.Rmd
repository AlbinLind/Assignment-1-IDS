...
We will also investigate how the z-score-method would perform. Given that the contains, a few, observations that are not unreasonable, but much larger than others, it might make more sense to use the z-score.
```{r}
...
ggplot()+geom_line(data=rolling_mean, mapping=aes(x=1:length(V1),y=V1,colour="red")) + geom_line(data=rolling_median, mapping=aes(x=1:length(V1),y=V1,colour="blue")) + geom_point(data=Data2_z_score_outliers, aes(x=1:length(V1),y=V1, colour="green"),alpha=0.2) + labs(x="Day",y="Claims (log10)",title = "Outliers removed via z-score-method", colour="Type") + scale_colour_hue(labels=c("median", "observation", "mean"))+ scale_y_log10()
```
